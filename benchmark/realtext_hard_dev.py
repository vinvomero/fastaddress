"""Hard-class dev tier: score a candidate vs v1 on the extended-ladder holdout.
...
Usage: python benchmark/realtext_hard_dev.py --candidate model/candidates/v44.crfsuite
"""
...
def main():
    ...
    for r, a, b in zip(rows, v1, cand):
        ...
        contrib.append(c)
        parts = r["origin"].split("-")
        by_div[parts[1] if len(parts) > 2 else "?"].append(c)
    ...
    ok = net > 0 and lo > 0
    print(f"\nHARD-CLASS DEV TIER  net positive, CI excludes zero : {'PASS' if ok else 'FAIL'}")
